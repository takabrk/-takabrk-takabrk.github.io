...
* オリジナルOS「Valkyrie SRX」などのUbuntu系Linuxディストリビューションのリマスター機能「Valkyrie Builder」（リマスターツール「Ubuntu Builder」をベースにGUI部分をGambas2からGlade(gtk+3)に変更）。各種情報を入力し、解凍したいISOを指定後に「Settings」ボタンを押します。その後、「Extract」ボタンを押す事で解凍が始まり、リマスターを行えます。リマスター作業が終われば、「Build」ボタンでビルドをします。
...
カスタムカーネル「VSRX kernel」をビルドする
-----------------------------------------------

カスタムカーネル「VSRX kernel」をビルドする機能で、カスタムカーネルは3種類のフレーバーがあります。

・VSRX kernel 4.16-pds　ー　CPUスケジューラーにPDS-MQ、I/OスケジューラーにBFQ-SQを使用。高レスポンス性とハイパフォーマンス性能を持つカーネルです。

・VSRX kernel 4.15-muqss　ー　CPUスケジューラーにMuQSS、I/OスケジューラーにBFQ-SQを使用。高レスポンス性とハイパフォーマンス性能を持つカーネルです。

・VSRX kernel 4.14-rt　ー　I/OスケジューラーにDeadlineを使用。PREEMPT_RTパッチを適用する事でハードリアルタイムカーネルに仕上がっています。

3種類のカスタムカーネルは、レスポンス性能を向上させる「Zen Interactive Tune」をONにしています。
...
「Change to the lightweight desktop enviroment」の項目を使いますと、私が開発したLinuxディストリビューション「VSRX」と同等のデスクトップ環境を構築する事が出来ます。
...
vipertools.pyを起動させて、「Install Penetration Tools」の項目にチェックをしてOKボタンを押すとインストールが可能です。
...
Handbrake CLIを使って、動画エンコードを行えます。

GUIでエンコードしたいソース、出力ファイル名、動画の解像度を入力します。

出力したファイルの動画コーデックはH.264、音声コーデックはMP3、コンテナはMP4となっています。
...
Guetzilは、Googleが開発したJPEGエンコーダーです。これを利用する事で、JPEGフォーマットの画像をほとんど変化なく、ファイル容量を削減する事が出来ます。
